  ...
  methods: {
    ...
    updateColumns() {
      ...
      const leafColumns = doFlattenColumns( notFixedColumns ),
        fixedLeafColumns = doFlattenColumns( states.fixedColumns ),
        rightFixedLeafColumns = doFlattenColumns( states.rightFixedColumns );
    ...
    cleanSelection() {
      ...
      if ( rowKey ) {
        ...
        const selectedMap = getKeysMap( selection, rowKey ),
          dataMap = getKeysMap( data, rowKey );
    ...
    _toggleAllSelection() {
      const states = this.states,
        ...
        value = states.selectOnIndeterminate ?
    ...
    updateSelectionByRowKey() {
      const states = this.states,
        { selection, rowKey, data } = states,
        selectedMap = getKeysMap( selection, rowKey );

      data.forEach( row => {
        const rowId = getRowIdentity( row, rowKey ),
          rowInfo = selectedMap[rowId];
    ...
    updateAllSelected() {
      const states = this.states,
        { selection, rowKey, selectable } = states,
      ...
      const isSelected = function( row ) {
        if ( selectedMap ) {
          return Boolean( selectedMap[getRowIdentity( row, rowKey )] );
        }
        return selection.indexOf( row ) !== -1;
      };
      ...
      for ( let i = 0, j = data.length; i < j; i++ ) {
        const item = data[i],
          isRowSelectable = selectable && selectable.call( null, item, i );
    ...
    clearFilter( columnKeys ) {
      const states = this.states,
        { tableHeader, fixedTableHeader, rightFixedTableHeader } = this.table.$refs;
