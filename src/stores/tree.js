  ...
  computed: {
    ...
    normalizedLazyNode() {
      const { rowKey, lazyTreeNodeMap, lazyColumnIdentifier } = this.states,
        keys = Object.keys( lazyTreeNodeMap ),
        res = {};
  ...
  methods: {
    ...
    updateTreeData() {
      const nested = this.normalizedData,
        normalizedLazyNode = this.normalizedLazyNode,
      ...
      if ( keys.length ) {
        const {
          ...
          rootLazyRowKeys = [],
          getExpanded = ( oldValue, key ) => {
            const included =
            defaultExpandAll ||
            expandRowKeys && expandRowKeys.indexOf( key ) !== -1;

            return Boolean( oldValue && oldValue.expanded || included );
          };
        ...
        if ( lazy && lazyKeys.length && rootLazyRowKeys.length ) {
          lazyKeys.forEach( key => {
            const oldValue = oldTreeData[key],
              lazyNodeChildren = normalizedLazyNode[key].children;
